                    ...
                    <section id="areaCorata" class="invoice">
                        ...
                        <div class="row invoice-info">
                            ...
                            <div class="col-sm-4 invoice-col">
                                ...
                                @endphp
                                <address>
                                    <strong>Total Dolar: </strong> {{ ($cajas->SumaTotalDolar + $cajas->SumaTotalDolarCredito) ?? '0.000' }} + {{$cajas->SumaTotalDolarCredito}}<br>
                                    <strong>Total Peso: </strong> {{ number_format(($cajas->SumaTotalPeso + $cajas->SumaTotalPesoCredito),2,'.',',') ?? '0.00' }}<br>
                                    <strong>Total Punto: </strong> {{ number_format(($cajas->SumaTotalPunto + $cajas->SumaTotalPuntoCredito),2,'.',',') ?? '0.00' }}<br>
                                    <strong>Total Transf: </strong> {{ number_format(($cajas->SumaTotalTransferencia + $cajas->SumaTotalTransferenciaCredito),2,'.',',') ?? '0.00' }}<br>
                                    <strong>Total Bolivar: </strong> {{ number_format(($cajas->SumaTotalBolivar + $cajas->SumaTotalBolivarCredito),2,'.',',') ?? '0.00' }}<br>
                                    @can('haveaccess', 'cajautilidad.show')
                                ...
                                <address>
                                    <div class="table-responsive">
                                        ...
                                        @can('haveaccess', 'cajatotalventa.show')
                                                <tr>
                                                    <th>Total Venta:</th>
                                                    <td>${{ $cajas->SumaTotalVentas + $cajas->SumaTotalVentasCredito ?? '0.000' }}</td>
                                                </tr>
                                        @endcan
                                        @can('haveaccess', 'cajautilidad.show')
                                                <tr>
