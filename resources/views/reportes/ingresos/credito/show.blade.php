...
@section('contenido')
    ...
    <div class="row">
        ...
        <div class="box-footer no-print">
            ...
            <a class="btn btn-success" href="{{route('reporte-ingreso-creditos')}}">{{__('Nueva Busqueda')}}</a>
            @endif
...
<section id="imprimir" class="invoice">
    ...
    @php
                        ...
                        if ($ing->status == 'Pendiente') {
                            ...
                            $Descuentos = "App\DetalleCreditoIngreso"::where('ingreso_id', $ing->id)->sum('descuento');
                            $Incrementos = "App\DetalleCreditoIngreso"::where('ingreso_id', $ing->id)->sum('incremento');
                            $totalCompras += $ing->precio_compra + $Incrementos - $Descuentos;
                            $operaconesC++;
                        }
                        @endphp
                ...
                <table class="table table-striped table-bordered table-condensed table-hover">
                    ...
                    {{-- {!! ' hola ' !!} --}}
                        ...
                        @endif
                            ...
                            @else
                            <?php
                            $descuento = "App\DetalleCreditoIngreso"::where('ingreso_id', $ing->id)->sum('descuento');
                            $incremento = "App\DetalleCreditoIngreso"::where('ingreso_id', $ing->id)->sum('incremento');

                            ?>
                                <td colspan="2"><b>Total compra:</b> {{ number_format($ing->precio_compra + $incremento - $descuento, 3,',','.') ?? '' }}</td>
                                <td> {{ $ing->status ?? '' }}</td>
                            @endif
                        ...
                        @endphp
                            ...
                            <tr style="{{$status}}">
                                ...
                                <table class="table table-striped table-bordered table-condensed table-hover">
                                    ...
                                    @foreach ($ing->pagos_credito_ingresos as $pago)
                                        <tbody>
                                            <td>{{$pago->caja ?? ''}}</td>
                                            <td>{{$pago->moto ?? ''}}</td>
                                            <td>{{$pago->abono ?? ''}}</td>
