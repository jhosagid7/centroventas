...
Route::resource('/descargos', 'DescargoController', ['except'=>[
    ...
    Route::get('/hola', function () {
        ...
        return $querys = Articulo::query()
        ->when(request('term'), function ($query, $term) {
            $query->select('id', 'nombre', 'codigo')
                ->selectRaw("match(nombre) against(? in natural language mode) as score", [$term])
                ->whereRaw("match(nombre) against(? in natural language mode) > 0.0000001", [$term]);
        })->get();
    })->name('hola');
